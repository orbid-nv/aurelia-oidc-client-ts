...
export default class OpenIdConnect {
  ...
  public configure(routerConfiguration: RouterConfiguration) {
    if (
      typeof routerConfiguration === "undefined" ||
      routerConfiguration === null
    ) {
      throw new Error(
        "routerConfiguration parameter must not be undefined or null"
      );
    }
  ...
  public async login(args: any = {}): Promise<void> {
    if (this.router.currentInstruction) {
      const loginRedirectValue =
        this.router.currentInstruction.queryParams[LoginRedirectKey];
  ...
  public async logout(args: any = {}): Promise<void> {
    try {
      ...
      if (err.message === "no end session endpoint") {
        ...
        this.logger.debug(
          "The user remains logged in at the authorization server."
        );
  ...
  public addOrRemoveHandler(key: keyof UserManagerEvents, handler) {
    if (!key.startsWith("add") && !key.startsWith("remove")) {
      let message =
        "The 'addOrRemoveHandlers' method expects a 'key' argument ";
  ...
  public observeUser(callback: (user: User) => void): Promise<void> {
    this.addOrRemoveHandler("addUserLoaded", () =>
      this.getUser().then(callback)
    );
    this.addOrRemoveHandler("addUserUnloaded", () =>
      this.getUser().then(callback)
    );
