...
export default class OpenIdConnectAuthorizeStep implements PipelineStep {
  ...
  public async run(
    ...
    if (
      this.requiresRole(navigationInstruction, OpenIdConnectRoles.Authenticated)
    ) {
      if (user === null || user.expired) {
        ...
        if (
          navigationInstruction.queryString &&
        ...
        const redirect = new Redirect(
          this.configuration.unauthorizedRedirectRoute + queryString
        );
